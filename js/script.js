...
document.getElementById('btn3')?.addEventListener('click', function () {
  codeEditor.value = codeSnippets['code3']
  output.innerHTML = codeSnippets['code3']
})

document.getElementById('btn4')?.addEventListener('click', function () {
  codeEditor.value = codeSnippets['code4']
  output.innerHTML = codeSnippets['code4']
})

document.getElementById('btn5')?.addEventListener('click', function () {
  codeEditor.value = codeSnippets['code5']
  output.innerHTML = codeSnippets['code5']
})
  ...
  if (mainLayout) {
    iconicWebsites.map((item, index) => {
      ...
      <div class="max-w-sm rounded-lg overflow-hidden shadow-lg bg-[var(--base-color)]">
          ...
          <div class="px-6 py-4">
              <div class="font-bold text-2xl mb-2 text-[var(--text-color)]">The Coldest Sunset</div>
          ...
          <div class="px-6 pt-4 pb-2 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
              <button class="bg-[var(--accent-color)] hover:bg-[var(--hover-color)] text-white font-semibold py-2 px-4 rounded-lg flex items-center justify-center space-x-2 custom-btn">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v8m0 0l-3-3m3 3l3-3m0 0V8m0 8V8" />
                  </svg>
                  <span>Read Article</span>
              </button>
              <button class="bg-[var(--line-color)] hover:bg-[var(--hover-color)] text-white font-semibold py-2 px-4 rounded-lg flex items-center justify-center space-x-2 custom-btn">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
